...
fs.readFile('1000-snapshots-overlap-with-Feb-2012.json', 'utf8', function(err, data) {
    ...
    exports.processSnapshots(JSON.parse(data));
    ...
    return {
        addProjectTime : function(objectID, febTimes) {
            ...
            if (projectTimeTotals.hasOwnProperty(objectID)) {
                 ...
                 *      projectTimeTotals[objectID].febTime / 1000 / 60 / 60 + " " +
                ...
                /*
                 * console.log(projectTimeTotals); console.log("Added new objectID of " + objectID + " : " +
                 * projectTimeTotals[objectID].febTime / 1000 / 60 / 60 + " " +
                 * projectTimeTotals[objectID].febBusinessTime / 1000 / 60 / 60);
                 */
 ...
 */
exports.processSnapshots = function(snapshots) {
    ...
    for (var i = 0; i < snapshots.length; i = i + 1) {
        ...
        //data validation, end times should be greater than start times.
        if(endTime >= startTime){

        // calculate schedule states and times for each entry
        // add it to the schedule states
        timeTotals.addScheduleStateAndTime(snapshots[i].ScheduleState, exports.getBusinessTime(startTime, endTime));
        ...
        if (startTime.getTime() <= febEnd.getTime() && endTime.getTime() >= febStart.getTime()) {
            ...
            febTotals = exports.processFebTime(startTime, endTime);
...
exports.processFebTime = function(sTime, eTime) {
    ...
    result.febTime = exports.getTimeElapsed(startTime, endTime);
    result.febBusinessTime = exports.getBusinessTime(startTime, endTime);

    return result;

};

exports.getTimeElapsed = function(sTime, eTime) {
    return eTime.getTime() - sTime.getTime();
};

//only for test
exports.getFebBusinessHours = function(sTime, eTime){
    if ((new Date(sTime)).getTime() <= febEnd.getTime() && (new Date(eTime)).getTime() >= febStart.getTime()) {
    var result = exports.processFebTime(new Date(sTime), new Date(eTime));
    return result.febBusinessTime;
    }
    else{
        return 0;
    }
...
exports.getBusinessTime =  function(sTime, eTime) {
    var startTime = new Date(sTime);
    var endTime = new Date(eTime);
    ...
    if (startTime.getDate() == endTime.getDate() && startTime.getYear() == endTime.getYear()) {
        if (endDay !== 0 && endDay !== 6) {
            //console.log("End time" + exports.getDaySplit(endTime, startOfBusinessDay, endOfBusinessDay));
            //console.log("Start time" + exports.getDaySplit(startTime, startOfBusinessDay, endOfBusinessDay));
            businessTime += exports.getDaySplit(startTime, startOfBusinessDay, endOfBusinessDay) -exports.getDaySplit(endTime, startOfBusinessDay, endOfBusinessDay) ;
    ...
    }else { // project spans more than one day
        ...
        // it is a partial day, must compute
            ...
            businessTime += exports.getDaySplit(startTime, startOfBusinessDay, endOfBusinessDay);
            /*
             * console.log("Partial first day: " + exports.getDaySplit(startTime, startOfBusinessDay,
        ...
        // it is a partial day, compute hours
            ...
            businessTime += workDayInMilliseconds - exports.getDaySplit(endTime, startOfBusinessDay, endOfBusinessDay);
            /*
             * console.log("Partial last day: " + ((8 * 60 * 60 * 1000) - exports.getDaySplit(endTime, startOfBusinessDay,
             * endOfBusinessDay))/1000/60/60);
             */
        }

        // figure out how many full days are in between
        businessTime += exports.getMiddleDays(startTime, endTime, endOfBusinessDay - startOfBusinessDay);
...
exports.getDaySplit = function(splitTime, startOfBusinessDay, endOfBusinessDay) {
    var timeElapse;
...
exports.getMiddleDays = function(sTime, eTime, hoursInTheDay) {
